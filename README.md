...
A custom integration for Home Assistant that connects to the [Waste Management (wm.com)](https://www.wm.com) API to pull the next pickup dates for your services (e.g., trash, recycling, yard waste). 
...
* **Optimized API Polling:** Previously, the integration would re-authenticate with the WM API for every single sensor on every single update. The code has been rewritten to establish a single authenticated session that is shared across all sensors, drastically reducing the number of requests and preventing potential rate-limiting or account lockouts.
